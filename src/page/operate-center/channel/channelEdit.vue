  ...
  export default {
    ...
    methods: {
      ...
      submitForm (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            // 编辑渠道
            const EditChannel = new Promise((resolve, reject) => {
              this.$http({
                method: 'post',
                url: window.config.server + '/api/v1/channels/channelId/channel',
                data: {
                  id: this.$route.params.channelId,
                  code: this.ruleForm.channelNumber,
                  name: this.ruleForm.channelName,
                  type: this.ruleForm.channelType,
                  mobile: this.ruleForm.contactNumber,
    ...
    created: function () {
      ...
      const ChannelInfo = new Promise((resolve, reject) => {
        this.$http({
          method: 'get',
          url: window.config.server + '/api/v1/channels/' + this.$route.params.channelId + '/channel',
          params: {},
          headers: {
            'languageCode': this.$route.params.lang,
            'Authorization': 'Bearer ' + this.$cookie.get('token')
